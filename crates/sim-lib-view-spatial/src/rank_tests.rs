...
fn spatial_collapses_periphery_glance_stays_one_item() {
    ...
    assert_eq!(
        field_sym_name(&ranked, "privacy-reason"),
        Some("camera-live")
    );

    let focus = panel(&ranked, "focus");
    assert_eq!(field_sym_name(focus, "attention-detail"), Some("foveal"));
    assert_eq!(access::field_bool(focus, "attention-lit"), Some(true));

    let near = panel(&ranked, "near");
    assert_eq!(field_sym_name(near, "attention-detail"), Some("peripheral"));
    assert_eq!(access::field_bool(near, "attention-lit"), Some(true));

    let over_budget = panel(&ranked, "over-budget");
    assert_eq!(
        field_sym_name(over_budget, "attention-detail"),
        Some("hidden")
    );
    assert_eq!(
        field_sym_name(over_budget, "attention-reason"),
        Some("budget")
    );
    assert_eq!(
    ...
    assert_eq!(field_sym_name(warrant, "attention-reason"), Some("pinned"));
    ...
    assert_eq!(field_sym_name(&glance, "urgency"), Some("error"));
...
fn field_sym_name<'a>(expr: &'a Expr, field: &str) -> Option<&'a str> {
    let Expr::Symbol(symbol) = access::field(expr, field)? else {
        return None;
    };
